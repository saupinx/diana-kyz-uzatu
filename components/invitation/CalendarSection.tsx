...
export function CalendarSection({ content, calendarUrl }: CalendarSectionProps) {
  return (
    <section className="reveal-section relative overflow-hidden px-5 py-7 text-center">
      <FloralDecor variant="sideBorder" className="floral-calendar-border" />
        ...
        <div className="premium-card mt-6 overflow-hidden rounded-[28px] p-5">
          <div className="relative z-10">
            <div className="flex items-end justify-between border-b border-neutral-200 pb-4">
              <span className="font-serif text-4xl text-black">05</span>
            ...
            <div className="mt-5 grid grid-cols-7 gap-1 text-center text-xs text-neutral-500">
              ...
              {days.map((day) => (
                <span
                  key={day}
                  className={`calendar-day grid aspect-square place-items-center rounded-full ${day === 5 ? "bg-black font-semibold text-white shadow-[0_10px_20px_rgba(0,0,0,0.18)]" : "text-neutral-700"}`}
                >
                  {day}
                </span>
              ))}
